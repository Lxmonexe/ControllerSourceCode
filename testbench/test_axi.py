...
async def clk_gen(dut,clk, period1_ns=10, period2_ns=8):  
    ...
    cocotb.start_soon(Clock(clk, period2_ns, units="ns").start()) #AXI clock
...
async def run_test(dut):
    ...
    axi_master = AxiLiteMaster(AxiLiteBus.from_prefix(dut, "s00_axi"), clk, rst,reset_active_level=False) # create an AxiLiteMaster instance
    ...
    await Timer(12000, units="ns")  # Wait to let the flash power up
